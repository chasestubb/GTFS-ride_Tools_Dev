...
function serviceStart(route, trips, frequencies){
    var arrival = 0;
    for (var j = 0; j < trips.length(); j++){
...
function serviceEnd(route, trips, frequencies){
    var end = 0;
    for (var j = 0; j < trips.length(); j++){
        var match2 = trips[j].route_id;
        var compare = match.localeCompare(route.route_id);
...
function serviceDays(route, trips, calendar){
    var days;
...
function countTripRiders(board_alights, trip){
    var count = 0;
...
function countStopRiders(board_alights, stop){
    var count = 0;
    for ( var i = 0; i < board_alights.length; i++){
...
function findTripRecordUse(board_alights, trips){
    var found;
    for ( var i = 0; i < board_alights.length; i++){
        var match = board_alights[i].trip_id;
        var compare = match.localeCompare(trip.trip_id);
...
function findStopRecordUse(board_alights, stop){
    var found;
    for ( var i = 0; i < board_alights.length; i++){
        var match = board_alights[i].stop_id;
        var compare = match.localeCompare(stop.stop_id);
